...
function getAllcolUrl(channelid) {
	...
	myRequest({
		...
		success: function(data) {
			...
			document.getElementById("sliderSegmentedControl").innerHTML = vhtml0;
		},
		fail: function(data) {}
	});
}
...
function requestListData(requesttype) {
	...
	myRequest({
		...
		success: function(data) {
			...
			if(requesttype == 0) { //刷新
				mui('#scroll0').pullRefresh().endPulldownToRefresh(); //refresh completed
				...
				mui('#scroll0').pullRefresh().endPullupToRefresh(obj.length == 0); //参数为true代表没有更多数据了。
			}
		...
		fail: function(data) {
			...
			if(requesttype == 0) { //刷新
				...
				mui('#scroll0').pullRefresh().endPullupToRefresh(); //参数为true代表没有更多数据了。
			}
		}
	});

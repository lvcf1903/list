...
function submitItem() {
	...
	submitmobile = document.getElementById("mobile").value; //手机号
	if(submitmobile.indexOf("*") != -1) {
		submitmobile = userInfo.mobile;
	}
	idcard = document.getElementById("idcard").value; //身份证号
	if(idcard.indexOf("*") != -1) {
		idcard = userInfo.idnum;
	}
	name = document.getElementById("applyname").value; //姓名
	if(name.indexOf("*") != -1) {
	...
	if(!(new RegExp("\\S+")).test(name)) { //判断字符串是否为空或都是空格
		alert("请输入姓名");
		return;
	} else if(!(new RegExp("\\S+")).test(idcard)) {
		alert("请输入身份证");
		return;
	} else if(reg.test(idcard) === false) {
		alert("身份证输入不合法");
		return;
	} else if(!(new RegExp("\\S+")).test(submitmobile)) {
		alert("请输入联系电话");
		return;
...
function getLicenseList(arg0, arg1, arg2) {
	...
	if(vsiteid == 64) {
		...
		var requesturl;
		requesturl = overallInterface + "ywzw/interface/sendCatalogInfo.do";
		setTimeout(function() {
			myRequest({
				url: requesturl,
				data: date,
...
function getlicense(code, type) {
	...
	requesturl = overallInterface + "ywzw/interface/licenseFile.do";
	//	}
	var heights1 = '';
	var requesturl;
	var innerhtml = '';
			...
			success: function(data) { //请求成功
				...
				if(data != null && data != "") {
					example2.items2.push({
						height: heights * 2,
						index: 0,
						imageurl: "http://220.191.228.153:9090/license/" + data.realpath,
						showurl: "http://220.191.228.153:9090/license/" + data.showpath
					});
					document.getElementById('loading').style.display = 'none';

...
function fillDatainView1(data) {
	...
	if(!(new RegExp("\\S+")).test(bz) || bz == null) {
		...
		document.getElementById("bztr").style.display = '';
	...
	document.getElementById("zxdh").href = "tel:" + zxdh.split('、')[0];
	document.getElementById("zxdh").innerHTML = zxdh.split('、')[0];
	document.getElementById("jdtsdh").href = "tel:" + jdtsdh.split('、')[0].split('或')[0];
	document.getElementById("jdtsdh").innerHTML = jdtsdh.split('、')[0].split('或')[0];
		...
		success: function(data) {
			...
			if((new RegExp("\\S+")).test(list)) { //有数据
				...
				if(isArray(vcai)) {
					...
					vcai = vcai.replace('14px','10.5pt');
					document.getElementById('declareinfo').innerHTML = "<div style='padding: 15px;font-size:16px;color: #222222;'>" + vcai + "</div>";
					return;
				}
			} else { //无数据
				dataArray = [];

...
function submitItem() {
	...
	for(var i = 0; i < materials.length; i++) {
		if(materials[i].necessity==1){
			is++;
		}
		for(var j = 0; j < filesArray.length; j++) {
			if(materials[i].material == filesArray[j].materialUNID && materials[i].necessity==1) {
				haveSubmit.push(materials[i].material);
			}
		}
	}
...
function jump() {
	...
	window.location.href = "odersuccess.html"+"?type=02&siteid=" + vsiteid+"&cityname="+itemname;
}
...
function requestWorkMaterial(itemcode, name, webid) {
	...
	myRequest({
		url:"https://app.zjzwfw.gov.cn/open/interfaces/shenbaocailiao.do",// zjzwurl + "interface/getBsznCailiaoList.do",
		data: date,
		header: '',

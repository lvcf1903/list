...
$(function() {
	...
	requesturl="https://tlzwfwzx.com/TL_HALL/rest/hall/statistic/itemblrs";
	$.ajax({
		type: "get",  
		url: requesturl,
		data:{
		...
		success: function(data) {
		    ...
		    listHtml +="<div  style='display: inline-block; border: 2px solid #F5F5F5;height: auto; width: 90%;'>"+
				...
				"<tr>"+
					"<td width='2%'></td>"+
					"<td width='20%' style=' height:32px; text-align: right;padding-right: 8px; background-color: ;'>在办人数</td>"+
					"<td width='2%'></td>";
					if(data[i].num==undefined){
				        listHtml +="<td id='yue' width='60%' style='color: black;text-align: left;padding-left: 10px;'></td>";
					}else{
				        listHtml +="<td id='yue' width='60%' style='color: black;text-align: left;padding-left: 10px;'>"+data[i].num+"</td>";
					}
					listHtml +="<td width='2%'></td>"+
				"</tr>"+
			"</table>"+

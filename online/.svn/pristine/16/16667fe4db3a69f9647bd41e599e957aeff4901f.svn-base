...
//请求网上预约列表
function requestOrderList(requesttype) {
	...
	myRequest({
			...
			if(dataArray.length > 0) { //有数据
				...
				for(var i = 0; i < dataArray.length; i++) {
					...
					if(onlinetype == "01") {
						...
						var ishaving = dataArray[i].ishaving;
						var img;
						if(ishaving == "false") {
							img = "../source/images/new_bs_sc.png";
						} else {
							img = "../source/images/new_bs_nosc.png"
						}
						...
						example.items.push({
							...
							itemcode: dataArray[i].id,
							department: dataArray[i].groupname,
							cntime: dataArray[i].cntime,
							lxmobile: lxdh,
							jdmobile: jddh,
							foruser: dataArray[i].foruser,
							lawLimit: dataArray[i].fdtime,
							img: img,
							ishaving: ishaving,
							xianshi: xianshi
						});
		...
		fail: function(data) {
			...
			document.getElementById("badmessage").style.display = "block";
			document.getElementById("listcontent").style.display = "none";
			//假数据  测试用 因为有跨域问题  先不删咯
			//			jiajson2 = JSON.parse(jiajson2);
			//			for(var i = 0; i < jiajson2.length; i++) {
			//					
			//						var lxdh = jiajson2[i].lxmobile.split("、")[0]; //取第一个电话
			//						var jddh = jiajson2[i].jdmobile.split("、")[0]; //取第一个电话
			//						example.items.push({
			//							index: i,
			//							name: jiajson2[i].name,
			//							itemcode: jiajson2[i].id,
			//							department: jiajson2[i].groupname,
			//							cntime: jiajson2[i].cntime,
			//							lxmobile: lxdh,
			//							jdmobile: jddh
			//						});
			//					
	...
	//			//############网上预约的列表暂时先写假数据############
	//			if(onlinetype == "01") { //01网上预约
	////				data = "[{\"id\":\"99\",\"name\":\"食品生产\"},{\"id\":\"100\",\"name\":\"食品经营\"},{\"id\":\"101\",\"name\":\"食药事项\"},{\"id\":\"102\",\"name\":\"企业注册\"},{\"id\":\"103\",\"name\":\"企业注册涉及后置审批\"},{\"id\":\"104\",\"name\":\"其他综合事务受理\"},{\"id\":\"105\",\"name\":\"投资项目综合受理\"},{\"id\":\"115\",\"name\":\"让证、发证\"},{\"id\":\"66\",\"name\":\"公积金缴存、提取\"},{\"id\":\"106\",\"name\":\"公积金贷款\"},{\"id\":\"107\",\"name\":\"中国联通\"},{\"id\":\"108\",\"name\":\"中国移动\"},{\"id\":\"109\",\"name\":\"中石油\"},{\"id\":\"110\",\"name\":\"中石化\"},{\"id\":\"111\",\"name\":\"华数电视\"},{\"id\":\"112\",\"name\":\"人才服务\"},{\"id\":\"113\",\"name\":\"中国电信\"},{\"id\":\"114\",\"name\":\"验审岗\"},{\"id\":\"116\",\"name\":\"高速ETC\"},{\"id\":\"125\",\"name\":\"交通\"},{\"id\":\"126\",\"name\":\"烟草\"},{\"id\":\"127\",\"name\":\"安监\"},{\"id\":\"128\",\"name\":\"质监\"},{\"id\":\"117\",\"name\":\"公交IC卡\"},{\"id\":\"118\",\"name\":\"工商代理\"},{\"id\":\"119\",\"name\":\"邮政速递\"},{\"id\":\"120\",\"name\":\"衢州日报\"},{\"id\":\"121\",\"name\":\"住建\"},{\"id\":\"122\",\"name\":\"卫计委\"},{\"id\":\"123\",\"name\":\"商务\"},{\"id\":\"124\",\"name\":\"联合办税\"}]";
	//			}
	//			document.getElementById('loading').style.display = 'none';
	//			var obj = JSON.parse(data);
	//			if(requesttype == 0) { //刷新
...
function soucang(inx, itemcode, name, lxmobile, jdmobile, lawLimit, foruser, cnqx, department) {
	...
	var json = {
		...
		code: itemcode,
		name: name,
		contactPhone: lxmobile,
		monitorComplain: jdmobile,
		lawLimit: lawLimit,
		foruser: foruser,
		promiseDay: cnqx,
		dept: department,
	...
	myRequest({
		url: overallInterface + 'submitCollect.do',
		data: json,
		header: '',
		success: function(data) {
			...
			mui.alert("收藏成功")
									requestOrderList(0);
					//alert(example.items[inx].img);
				example.items[inx].img = "../source/images/new_bs_nosc.png";
				example.items[inx].ishaving = "true";
			} else {
				mui.alert("收藏失败")
			}
...
function quxiao(inx, itemcode, name, lxmobile, jdmobile, lawLimit, foruser, cnqx, department) {
	var json = {
		phone: wxuserInfo.mobile,
		code: itemcode,
	...
	myRequest({
		url: overallInterface + 'cancelCollect.do',
		data: json,
		header: '',
		success: function(data) {
			...
			if(data.result == "true") {
				mui.alert("取消收藏成功");
									requestOrderList(0);
				example.items[inx].img = "../source/images/new_bs_sc.png";
				example.items[inx].ishaving = "false";
				//example.items.img="../source/images/new_bs_nosc.png"
			} else {
				mui.alert("收藏失败")
			}

...
function onChooseImage(arg0, arg1, arg2) {
  if(window.localStorage.ydswryyhyjhhspb && arg2 == "异地死亡人员运回原籍火化审批表"){
    console.log(window.localStorage.ydswryyhyjhhspb)
    return
  }
	//调用我们的方法
	try {
			...
			success: function(data) {
				...
				if(data.result == 'true') {
					...
					for(var i = 0; i < data.picPath.length; i++) {
						...
						filesArray.push(dic);
            console.log(dic,"图片");
            if(arg2 == "异地死亡人员运回原籍火化审批表"){
              window.localStorage.ydswryyhyjhhspb1 = true;
              window.localStorage.removeItem("ydswryyhyjhhspb");
            }

					}
				} else if(data.result == 'cancel') {} else {
					alert("上传失败");
				}
			},
			fail: function(data) {
...
function getUserInfoSuccess(data) {
	...
	if(colid == -1) {
		...
		requestOrderList(0, colid, keyword, usertype);
	}

...
function requestMatter(itemcode) {
    myRequest({
        ...
        success: function (data) {
            //console.log("44=="+JSON.stringify(data))
...
function submitItem() {
    ...
    if ((new RegExp("\\S+")).test(vbdcl)) {
        ...
        $.ajax({
            ...
            //	url: 'http://192.168.88.131:8080/zjtywssb/ywzw/interface/submitOnlineHandle.do',
            url: overallInterface + 'ywzw/interface/submitOnlineHandle.do',
            data: pramaData,
            dataType: 'json',
            success: function (data) {
...
function jump() {
    //if(dregioncode = "330781") {
    window.location.href = "odersuccesss.html?type=02&siteid=" + vsiteid + "&cityname=" + cityName1;
    //} else {
...
function jump1() {
    //	alert(dregioncode);
    //	if(dregioncode = "330781") {
    window.location.href = "odersuccesss.html?type=03&siteid=" + vsiteid + "&cityname=" + cityName1;

new Vue({
    ...
    created:function(){
        ...
        if(sxjwwbhdwxsxksqb != undefined){
            let bdarray = JSON.parse(localStorage.getItem("sxjwwbhdwxsxksqb")).wordform[0];
    ...
    methods:{
        saveGo(){
            if(!$('.chengruo input').prop('checked') || !$('#txt_sxjwwbhdwmc').val() || !$('#txt_nd').val() || !$('#txt_gbpc').val() || !$('#txt_dz').val()
            ...
            pramadic["txt_title"] = ['form_sxjwwbhdwxsxksqb'];

            pramadic['txt_sxjwwbhdwmc'] = '[\"市县级文物保护单位名称",\"'+$('#txt_sxjwwbhdwmc').val()+'\"]';
            ...
            window.localStorage.sxjwwbhdwxsxksqb = formXml;
            window.history.go(-1);
        }
    }
})